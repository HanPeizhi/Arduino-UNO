...
})});if(typeof tinyMCE!="undefined"){$(".s_grade_item_assignment #edit-comment.s-tinymce-load-editor:not(.sGradeItem-processed), .s_grade_item_assessment #edit-comment.s-tinymce-load-editor:not(.sGradeItem-processed)",a).addClass("sDiscussion-sGradeItem").each(function(){var e=$(this),d=e.attr("id"),c=tinyMCE.get(d);if(!c){sTinymceInit({elements:d,toolbar:"basic_comment"})}})}$("#s-grade-item-add-attachment-form:not(.sGradeItem-processed)",a).addClass("sGradeItem-processed").each(function(){setTimeout(function(){$("#file-selector").click()},150)});var b="";$(".comment-area-toggle").on("click",function(){b=$("#edit-comment-wrapper .title-infield").val()});$(".grade-submit-buttons .cancel-btn").on("click",function(c){$("#edit-comment-wrapper .title-infield").val(b);$(c.currentTarget).closest("#s-grade-item-edit-enrollment-grade-form").removeClass("active")});if($("body").hasClass("s-enable-mathml")){s_renderMath()}$("body:not(.sGradeItem-processed)",a).addClass("sGradeItem-processed").each(function(){var d;
var c;$(this).on("grade-item-disable-top-controls",function(){$(".grade-item-action-links .action-links-unfold",a).addClass("disabled");var f=$(".link-btn.view-info",a);if(f.length){if(f.data("events")){d=f.data("events").click;f.data("events").click=null}f.addClass("disabled");f.tipsy("disable")}var e=$(".link-btn.view-stats",a);if(e.length){if(e.data("events")){c=e.data("events").click;e.data("events").click=null}e.addClass("disabled");e.tipsy("disable");e.click(function(g){g.preventDefault()})}$(".assessment-status-wrapper",a).addClass("disabled")});$(this).on("grade-item-enable-top-controls",function(){$(".grade-item-action-links .action-links-unfold",a).removeClass("disabled");var f=$(".link-btn.view-info",a);if(f.length){f.removeClass("disabled").tipsy("enable");if(f.data("events")){f.data("events").click=d}}var e=$(".link-btn.view-stats",a);if(e.length){e.removeClass("disabled").tipsy("enable");if(e.data("events")){e.data("events").click=c}}$(".assessment-status-wrapper",a).removeClass("disabled")
})})};function sGradeItemEnableCommentJump(){$(".go-to-reply").click(function(){var a=$(this).parent().parent().parent().parent();if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var b=$(this.hash);$(b).effect("highlight",{color:"#f9b974"},3000);b=b.length&&b||$("[name="+this.hash.slice(1)+"]");if(b.length){var c=b.offset().top-20;$("html,body").animate({scrollTop:c},500);return false}}})}Drupal.Ajax.plugins.s_grade_item=function(l,k){if(l=="message"){var c=k.local.submitter;formObj=c.closest(".post-comment-form");if(formObj.length){if(typeof k.redirect=="string"&&k.redirect.length>0){window.location.href=k.redirect}else{var e=k.ajax_validate_output;var a=k.ajax_submit_output;var b="comment-container";var j="";if(a!=undefined){j=a}else{if(e!=undefined){j=e}}if($("."+b+" #s_comments").length>0){$("."+b+" #s_comments").fadeIn(5000,function(){var m=$(j);$("."+b+" #s_comments").append(m);Drupal.attachBehaviors(m)})}else{var f='<div id="s_comments">'+j+"</div>";
var h=$(f);$(".no-comments").replaceWith(h);Drupal.attachBehaviors(h)}var d=formObj.find(".s-tinymce-load-editor");if(d.length){var g=tinyMCE.get(d.attr("id"));if(g){g.setContent("");g.save()}}return false}}}return false};var wait_image="/sites/all/themes/schoology_theme/images/ajax-loader.gif";var wait_image_width=43;var wait_image_height=11;Drupal.Ajax.plugins.s_comment=function(h,b){switch(h){case"submit":var m=b.submitter,l=m.closest("form"),x=l.find(".s-tinymce-load-editor");if(l.hasClass("post-comment-form")){m.attr("disabled","disabled").parent().addClass("disabled")}if(x.length){var c=tinyMCE.get(x.attr("id"));if(c){c.save()}}break;case"message":var m=b.local.submitter;var d=m.parents("form");if(!m.parents("form").hasClass("post-comment-form")){return}var z=(m.parents(".submit-enabled").length>0);if(z){m.attr("disabled",false).parent().removeClass("disabled")}var D=b.ajax_validate_output;var k;var v;if(typeof(b.ajax_submit_output)=="object"){k=b.ajax_submit_output.newComment;v=b.ajax_submit_output.allComments
}else{k=b.ajax_submit_output}var u=m.closest(":has(.feed-comments, .blog-comments, .discussion-content, .album-comments, .update-comments)").find(".feed-comments, .blog-comments, .discussion-content, .album-comments, .update-comments");var y=u.parent();var w="";var j=$("body").hasClass("discussion-view");if(k!=undefined){w=k}else{if(D!=undefined){w=D}}$("p.no-discussion",$(".discussion-content")).hide();if(u.find("#s_comments").length>0){u.find("#s_comments").fadeIn(5000,function(){var G=$("<div class='discussion-card'></div>");G.append(w);if(j){$(this).prepend(G)}else{$(this).append(G)}Drupal.attachBehaviors(G);var F=false;for(i=0;i<b.messages_error.length;++i){if(b.messages_error[i].value=="This discussion is locked"){F=true}}if(F){u.find("textarea").attr("defaulttext",u.find("textarea").val()).addClass("is-locked")}sCommentScrollToNewComment(G,750,":has(.s-comments-post-form-new)");if(j){var E="rgba(213,227, 241, 0.6)";G.effect("highlight",{color:E},3000)}})}else{var s=$('<div id="s_comments"><div class="discussion-card">'+w+"</div></div>");
if(j){var B="rgba(213,227, 241, 0.6)";u.find(".s-comments-post-form").after(s);s.effect("highlight",{color:B},3000)}else{u.find(".s-comments-post-form").before(s)}Drupal.attachBehaviors(s)}var f=d.find("textarea");if(f.length){if(f.hasClass("s-tinymce-load-editor")){var c=tinyMCE.get(f.attr("id"));if(c){c.setContent("");c.save()}}else{f.val("").trigger("blur")}}if(v){for(var g in v){var o=u.find("#comment-"+g);var C=$(".comment-body-wrapper",o);var q=$(".comment-top",o);C.removeClass("disabled");var t=$("<textarea/>").html(v[g].body).val();C.html(t);$(v[g].attachments).insertAfter(q);o.removeClass("disabled");$(".comment-reply",o).removeClass("disabled").show()}Drupal.attachBehaviors(u);u.prev(".discussion-require-post-notice").remove();var n=u.parents(".course-discussion");$("#discussion-user-stats-wrapper",n).removeClass("disabled");sAttachBehaviors(["s_discussion"],n)}if(u.hasClass("update-comments")){popup=Popups.activePopup();if(popup){Popups.resizeAndCenter(popup)}}var r=y.find(".feed-footer").find("span.ajax-post-comment");
var p=r.text();var A=p.split(" ");var e=A[0];var a=A[1];if(e==1){a=A[1]+"s"}++e;r.text(e+" "+a);$("#edit-comment-wrapper #edit-comment",d).css("height","32px");break;case"afterMessage":if(!b.ajax_submit_output){return}if($("#attachments").length>0){resetAttachmentForm();sAttachmentResetBasicUploader();unWantedForm=$("#edit-file-swf-wrapper .upload-form");if(unWantedForm){$("#edit-file-swf-wrapper").append(unWantedForm.children("input"));unWantedForm.remove()}}if($("li#file-selector").hasClass("active")){$("li#file-selector").removeClass("active")}break}};var wait_image="/sites/all/themes/schoology_theme/images/ajax-loader.gif";var wait_image_width=43;var wait_image_height=11;Drupal.Ajax.plugins.s_comment_reply=function(l,b){switch(l){case"submit":var q=b.submitter;q.attr("disabled","disabled").parent().addClass("disabled");break;case"message":var q=b.local.submitter;var a=q.parents("form");if(a.attr("id")=="s-comment-reply-form"){if(typeof b.redirect=="string"&&b.redirect.length>0){window.location.href=b.redirect
}else{q.attr("disabled",false).parent().removeClass("disabled");var x=b.ajax_validate_output;var o=b.ajax_submit_output;var s="";if(o!=undefined){var h=a.find("textarea");if(h.length){if(h.hasClass("s-tinymce-load-editor")){var g=tinyMCE.get(h.attr("id"));if(g){g.setContent("");g.save()}}else{h.val("").trigger("blur")}}s=o;var f=q.parents(".comment").eq(0);var t=f.next(".s_comments_level");if(t.length==0){t=$('<div class="s_comments_level"></div>');var u=f.parent(".s_comments_level");if(u&&u.hasClass("nested-threshold-exceeded")){t.addClass("nested-threshold-exceeded")}f.after(t)}var d=$(s);t.append(d);a.parent().hide();var m=$("body").hasClass("discussion-view");var w="#f9b974";sCommentScrollToNewComment(d,500);if(m){w="rgba(213,227, 241, 0.6)"}d.effect("highlight",{color:w},3000);Drupal.attachBehaviors(t)}else{if(x!=undefined){s=x}}var j=q.closest(".thread-root");var n=j.prev();var r=$(s);var e=$(".pending-comment",r).length>0;if(n.hasClass("expander-bar")&&!e){var k=$(".num-replies",n);
var v=k.text();var c=$(".expander-text",n);var p=$(".expander-hide-text",n);v=parseInt(v);v++;k.html(v);c.html(Drupal.formatPlural(v,"1 Reply","@count Replies"));p.html(Drupal.formatPlural(v,"Hide 1 reply","Hide All @count Replies"));if(v==1){$(".expander-link-expanded",n).removeClass("hidden")}}}}break}};Drupal.theme.s_course_course_dropdown=function(d){var c="";var b={attributes:{title:Drupal.t("See All Courses")}};var a={attributes:{title:Drupal.t("Browse Courses")}};c+='<div class="s-dropdown course-dropdown" ng-controller="s_course_active_courses">';c+='<img ng-show="loading" class="ajax-loader" src="/sites/all/themes/schoology_theme/images/ajax-loader.gif" alt="'+Drupal.t("Loading")+'" />';c+=Drupal.theme.s_course_course_dropdown_list();c+='<div class="courses-action no-active-courses-item" style="display:none" ng-hide="loading">';c+='<span ng-bind-html-unsafe="create_course_link"></span>';c+='<span ng-bind-html-unsafe="join_course_link"></span>';c+='<span class="wrapper-for-actions">';
c+='<span tabindex="0" role="button" title="'+Drupal.t("Reorder Courses")+'" class="reorderRealms" realm="course" ng-hide="courses.sections.length < 2">Reorder</span>';c+='<span class="see-all" title="'+Drupal.t("Browse Courses")+'" ng-show="!courses.sections.length && permissions.can_browse_courses">'+Drupal.l(Drupal.t("Browse"),"/courses/browse",a)+"</span>";c+='<span class="see-all" title="'+Drupal.t("See All Courses")+'" ng-hide="!courses.sections.length && permissions.can_browse_courses">'+Drupal.l(Drupal.t("See All"),"/courses",b)+"</span>";c+="</span>";c+="</div>";c+='<span class="load-watch" loaded="loaded"></span>';c+="</div>";return c};Drupal.theme.s_course_course_dropdown_list=function(c,b){var a="";a+='<ul class="courses-listing listing mycourses_block '+(b?"course-reorder":"")+'" role="menu" ng-class="{\'show-buildings\':courses.use_buildings_names}">';a+='<div class="sections-list">';a+='<li id="{{section.nid}}" ng-repeat="section in courses.sections" class="section-item '+(b?"draggable":"")+'" ng-class="{\'admin-section\':section.is_admin}">';
a+='<span class="drag-handle drag-handle-ng" ng-hide="!show_handle"></span>';a+='<div class="section-item-left" ng-bind-html-unsafe="section.logo">';a+="</div>";a+='<div class="section-item-right" ng-class="{\'requires-verification\': (section.is_k6 && section.is_admin && !permissions.is_verified)}">';a+='<div class="is_k6" ng-show="section.is_k6 && section.is_admin && !permissions.is_verified"><a class="requires-verification-link" href="/verify"><span class="s-infotip" text="k6_infotip" item-info="section" gravity="w" enabled="true"><span class="inline-icon medium verification-error"></span></span></a></div>';a+='<a href="{{section.link}}" role="menuitem">';a+='<span ng-switch on="section.section_override_roles_enabled" class="listing-icons course-icons">';a+="<span ng-switch-when=\"false\" ng-class=\"{'admin-icon':section.admin_icon, 'course-icon':!section.admin_icon}\"></span>";a+="<span ng-switch-when=\"true\" ng-class=\"{'admin-icon admin-icon-shield admin-icon-shield--star':section.admin_icon && !section.section_override_role_present, 'admin-icon admin-icon-shield':section.admin_icon && section.section_override_role_present, 'course-icon':!section.admin_icon}\"></span>";
a+="</span>";a+='<div ng-switch on="section.infotip" class="s-course-title">';a+='<span ng-switch-when="false">{{section.title}}</span>';a+='<span ng-switch-when="true"><span class="s-infotip" text="section.infotip_text" item-info="section" gravity="section.infotip_gravity" enabled="true">{{itemInfo.title}}</span></span>';a+="</div>";a+='<div ng-switch on="courses.use_buildings_names">';a+='<div ng-switch-when="true" class="s-section-building-title">{{section.building_title}}</div>';a+="</div>";a+="</a>";a+="</div>";a+="</li>";a+="</div>";a+='<li class="no-active-courses-item" style="display:none" ng:hide="loading || courses.sections">';a+='<div class="no-courses">';a+=Drupal.t("You have no active courses");a+="</div>";a+="</li>";a+="</ul>";if(b){a+='<input type="hidden" class="course-reorder-weights" value="" />'}return a};sAngular.addController("s_course_active_courses",["$scope","Course",function(a,b){a.courses=null;a.permissions=null;a.loading=true;a.k6_infotip=Drupal.t("Schoology requires all instructors teaching students under the age of 13 to verify their identities. Students under the age of 13 cannot register for this course until you have been verified. Click here to verify your account.");
a.load=function(c){if(a.courses===null){b.getActiveCourses(function(d,f){a.courses=d;a.permissions=f;a.loading=false;var e={};$.each(d.courses,function(h,g){e[g.nid]=h});a.courses.use_buildings_names=false;$.each(d.sections,function(j,l){var h=e[l.course_nid];var g=d.courses[h];l.infotip=false;l.title=g.course_title+": "+l.section_title;if(!g.same_school||!g.same_building){a.courses.use_buildings_names=true}l.building_title=g.building_title});$.each(d.courses,function(h,g){g.sections.sort(function(k,j){return k.section_title>j.section_title})});if(a.permissions.can_join_courses){a.join_course_link=Drupal.popup(Drupal.l(Drupal.t("Join"),"/enroll/course",{attributes:{"class":"action-join",title:Drupal.t("Join Course")}}),{extraClass:"popups-small",updateMethod:"reload",hijackDestination:false,updateSource:"final",doneTest:/course\/\d+?|home/,ajaxForm:true})}if(a.permissions.can_create_courses){if(a.permissions.school_has_grading_periods){a.create_course_link=Drupal.popup(Drupal.l(Drupal.t("Create"),"/course/create",{attributes:{"class":"action-create",title:Drupal.t("Create Course")}}),{extraClass:"popups-large popups-section",updateMethod:"reload",hijackDestination:false,updateSource:"final",doneTest:/course\/\d+/,ajaxForm:true})
