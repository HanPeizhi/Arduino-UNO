 ...
 * 4.1 Use Long Term Storage to Remember a Colour Selection.
  ...
  Download the StoreHue_Exercise.zip zipped sketch folder.
  Upload it to your Arduino.
  When you press the button on your circuit, the colour of the LED will cycle smoothly.
  Modify the sketch such that:
  1. When you release the button the value of hue is stored in EEPROM.
  2. When the sketch first starts, the value of hue is read from EEPROM 
      and the LED starts with the same colour as was displayed when the sketch was stopped. 
      This should work even if you unplug your Arduino. 
      Just in case some other sketch wrote to that location, do a sanity check on the data you read in. 
      If it is outside the range 0 — 359 then set it to 0.
...
void loop()
{
  ...
  //If it is outside the range 0 — 359 then set it to 0.
  if(hue < 0 || hue >359) hue = 0;
  
  //If button was just released, store hue
  if (curButton == HIGH && lastButton == LOW)
  {
    EEPROM.put(0, hue);
    Serial.println(sizeof(float)); //INTERVAL: int is 2, char is, float 4
