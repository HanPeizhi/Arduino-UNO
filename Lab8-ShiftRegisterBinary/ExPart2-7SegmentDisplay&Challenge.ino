 ...
 *  Peizhi Han
 *    Lab8:
 *    - Part 1: Part 1: Chaining: chain two shift registers together 
 *                        so that your output will be 16 bits (or 2 bytes)
 *    - Remember that one bit is shifted out (on pin 9), 
 *        and you want it to shift that value into the DS (pin 14) of the second shift register. 
 *         In other words, connect pin 9 of the first register to pin 14 of the second shift register.
 *
 *    THU NOV 9 2017
 *
 */
...
void loop()                     // run over and over again
{
  int delayTime = 100; //the number of milliseconds to delay between LED updates
   updateLEDsPattern(pattern,16);
   delay(delayTime); 
   updateLEDsPattern(pattern1,16);
      delay(delayTime); 
   updateLEDsPattern(pattern2,16);
      delay(delayTime); 
   delay(delayTime); 
